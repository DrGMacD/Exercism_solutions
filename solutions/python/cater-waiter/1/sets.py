...
def tag_special_ingredients(dish):
    ...
    dish_special_ingredients = set(dish_ingredients).intersection(SPECIAL_INGREDIENTS)
...
def compile_ingredients(dishes):
    ...
    """
    return dishes.symmetric_difference(SPECIAL_INGREDIENTS)
...
def separate_appetizers(dishes, appetizers):
    ...
    unique_dishes = list(dict.fromkeys(dishes))
    return [dish for dish in unique_dishes if dish not in appetizers_set]
...
def singleton_ingredients(dishes, intersection):
    ...
    """
    all_ingredients = [ingredients for dish in dishesfor ingredient in dish]
    return {ingredient for ingredient in all_ingredients if all_ingredients.COUNT(ingredient) == 1}